                  ...
                  <div style={{ marginTop: 12 }} data-testid="photo-gallery">
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                      {fitness.photos.slice(0, 6).map((p) => (
                        <a key={p.id} href={`${process.env.REACT_APP_BACKEND_URL}${p.url}`} target="_blank" rel="noreferrer" data-testid={`photo-link-${p.id}`}>
                          <div className="glass" style={{ borderRadius: 12, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.10)' }}>
                            <img alt="progress" src={`${process.env.REACT_APP_BACKEND_URL}${p.url}`} style={{ width: '100%', height: 90, objectFit: 'cover', display: 'block' }} />
                          </div>
                        </a>
                      ))}
                    </div>
